...
class GeminiService:
    ...
    async def submit_batch_job_async(
        ...
        return {
            ...
            "submitted_at": datetime.now(UTC).replace(tzinfo=None).isoformat()
        }
        ...
        await collection.update_one(
            {"trade_type": trade_type},
            {
                "$set": {
                    ...
                    "batches_submitted_at": datetime.now(UTC).replace(tzinfo=None),
        ...
        for aspect_ratio, job_info in batch_jobs.items():
            ...
            if status["is_complete"]:
                if status["is_success"]:
                    ...
                    batch_jobs[aspect_ratio]["completed_at"] = datetime.now(UTC).replace(tzinfo=None).isoformat()
                    succeeded_count += 1
                else:
    ...
    async def process_batches_for_trade(
        ...
        for aspect_ratio, job_info in batch_jobs.items():
            ...
            try:
                ...
                batch_jobs[aspect_ratio]["processed_at"] = datetime.now(UTC).replace(tzinfo=None).isoformat()
        ...
        await collection.update_one(
            {"trade_type": trade_type},
            {
                "$set": {
                    ...
                    "generation_completed_at": datetime.now(UTC).replace(tzinfo=None) if all_processed else None,
                    "updated_at": datetime.now(UTC).replace(tzinfo=None)
                }
            }
        )
