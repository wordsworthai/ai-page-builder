/**
 * Hook to trigger content regeneration flow with credit validation.
 * 
 * Uses the credit-based pricing system where content regeneration costs 3 credits.
...
/**
 * Hook to trigger content regeneration
 * 
 * @example
 * const { mutateAsync, isPending, error } = useRegenerateContent();
 * 
...
export const useRegenerateContent = () => {
  ...
  return useMutation<GeneratePageResponse, any, RegenerateContentRequest>({
    mutationFn: async (request: RegenerateContentRequest) => {
      try {
        const response = await fetch(
          `${OpenAPI.BASE}/api/generations/${request.generationVersionId}/regenerate-content`,
          {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
          ...
          throw createGenerationError({
            status: response.status,
            detail: errorData.detail || 'Failed to start content regeneration',
          });
        }
        ...
        // Handle other errors
        throw createGenerationError(error, 'Failed to start content regeneration');
      }
    },
    ...
    onSuccess: (data) => {
      ...
      createSnackBar({
        content: 'Content regeneration started successfully!',
        severity: 'success',
        autoHide: true
      });
    },
    
    onError: (error: GenerationError, variables: RegenerateContentRequest) => {
